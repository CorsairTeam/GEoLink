...
def create_treeview_points(viewer):
    ...
    ToolTip(points_title_label, "●  Clic sur nom du point                : Mise à jour des champs\n"
                                "●  Double-clic sur nom du point  : Centrer la carte sur le point\n"
                                "●  Clic sur case à cocher              : Sélectionner le point\n" )

    # Création du Treeview
    viewer.tree = ttk.Treeview(viewer.treeview_frame, columns=("selected", "nom"), show="headings", height=10)
    viewer.tree.heading("selected", text="Select")
    ...
    viewer.tree.bind("<Double-1>", lambda event: viewer.mbtiles_manager.on_point_tree_double_click(event))  # Centre l'affichage sur le point sélectionné
    ...
    btn_select_all = ttk.Button(
        ...
        text="Select All",
        width=16,
        command=lambda: utility.select_all_treeview(viewer.points_checked_items, viewer.tree),
    ...
    btn_deselect_all = ttk.Button(
        ...
        text="Deselect All",
        width=16,
        command=lambda: utility.deselect_all_treeview(viewer.points_checked_items, viewer.tree),
    )  # fonction d'attente pour les commandes du menu
...
def update_input_frame_points(viewer):
    ...
    # Couleur
    viewer.point_color_label = tk.Label(viewer.input_frame, text="Couleur : ", width=18, anchor="w")
    viewer.point_color_label.grid(row=2, column=0, padx=5, pady=5)
    ToolTip(viewer.point_color_label, "Couleur du point dans Foreflight.")
    colors = [
        "Rouge",
        "Vert",
        "Bleu",
    ...
    # Type de points
    viewer.type_points_label = tk.Label(viewer.input_frame, text="Type de points : ", width=18, anchor="w")
    viewer.type_points_label.grid(row=3, column=0, padx=5, pady=5)
    ToolTip(viewer.type_points_label, "Forme du point dans Foreflight.")
    type_points = ["Punaise", "Cible", "Panneau", "Carré", "Cercle", "Triangle", "Diamant"]
    ...
    if viewer.creation_mode.get() == "coordonnees":
        ...
        if viewer.coord_type.get() == "Degrés":
            # Remplissage du frame de coordonnées en degrés
            fill_coordinates_degrees_frame(viewer)       

        elif viewer.coord_type.get() == "Degrés/Minutes":
            # Remplissage du frame de coordonnées en degrés/minutes
            fill_coordinates_degrees_minutes_frame(viewer)       

        elif viewer.coord_type.get() == "Degrés/Minutes/Secondes":
            # Remplissage du frame de coordonnées en degrés/minutes/secondes
            fill_coordinates_degrees_minutes_seconds_frame(viewer)

        elif viewer.coord_type.get() == "Calamar":
            # Remplissage du frame de coordonnées en calamar
            fill_coordinates_calamar_frame(viewer)
            
    elif viewer.creation_mode.get() == "radial":
        #Remplissage du frame de coordonnées en radial
        fill_coordinates_radial_frame(viewer)

    elif viewer.creation_mode.get() == "click":
        # Remplissage du frame de creatio de point sur la carte
        fill_map_frame(viewer)
    
    # Réinitialiser les points cliqués, la ligne temporaire et le point cliqué
    viewer.clicked_points = []
    viewer.mbtiles_manager.clear_temp_line()
    
    if hasattr(viewer, "clicked_point"):
        del viewer.clicked_point
    viewer.mbtiles_manager.clear_temp_point()
...
def setup_points_tabs(viewer):
    ...
    viewer.input_frame.pack(fill=tk.X, side=tk.TOP, padx=5, pady=5)
